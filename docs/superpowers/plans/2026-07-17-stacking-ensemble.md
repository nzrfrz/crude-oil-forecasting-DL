# Task 5, Stacking Ensemble Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Tulis `12-stacking-ensemble.py`, cari kombinasi terbaik dari 8 base learner (satu per arsitektur, varian tuned/fixed dipilih otomatis) lewat exhaustive search ukuran 3-5 anggota x 2 meta-learner, evaluasi kombinasi pemenang, lalu perbarui `todo-list.md` dan `final-report.md` dengan hasilnya.

**Architecture:** Satu script baru, tidak ada retrain, cuma load checkpoint yang sudah ada (`models/{full,wu}/*.pt`), generate prediksi di val/test/(unseen), cari kombinasi terbaik pakai MAE validation set, evaluasi pemenang di test/(unseen), tulis laporan `.md` + plot + simpan meta-learner. Dijalankan dua kali (`--run wu` lalu `--run full`) oleh user sendiri di cmd dengan venv aktif, sama seperti semua stage sebelumnya.

**Tech Stack:** venv `D:\Coding\#bigdata\venv` (Python 3.11.9, torch 2.5.1+cu121, sklearn 1.8.0, pandas, matplotlib, joblib). Referensi spec, `docs/superpowers/specs/2026-07-17-stacking-ensemble-design.md`.

## Global Constraints

1. Tidak ada retrain base learner, hanya load checkpoint `models/{full,wu}/*.pt` yang sudah ada.
2. Penulisan `.md` TIDAK memakai tanda "-", "--", "---" sebagai pemisah kalimat naratif, ganti titik/koma, kalimat ber ":" dijadikan list baris baru (bullet list markdown `-` sebagai struktur tetap boleh).
3. Commit git tanpa baris co-author.
4. Semua perintah `python ...` dijalankan sendiri oleh user di cmd dengan venv aktif, agent berhenti di step "Run" dan menunggu laporan hasil sebelum lanjut ke verifikasi.
5. Meta-learner (RidgeCV) difit pada prediksi base learner dalam skala USD asli (inverse `scaler_y`), bukan skala scaled, mengikuti preseden `D:\Coding\#bigdata\crude-oil-forecasting-DL\07-stacking-ensemble.py` (bobot Ridge jadi langsung interpretable dalam USD).
6. Kriteria rangking 364 entri kombinasi memakai MAE validation set, bukan test set.

---

### Task 1: Tulis `12-stacking-ensemble.py`

**Files:**
- Create: `12-stacking-ensemble.py`

**Interfaces:**
- Consumes: `dataset/splits/<tag>/splits.npz` (`X_train, y_train, X_test, y_test, dates_test`, plus `X_unseen, y_unseen, dates_unseen` untuk `full`), `dataset/scalers/<tag>/scaler_X.pkl`, `scaler_y.pkl`, checkpoint `models/<tag>/{mlp,mlp_tuned,rnn,lstm,bilstm,gru,tcn,transformer,transformer_recency_fixed,informer,informer_recency_fixed}_model.pt`.
- Produces: `evaluations/statistical/stacking/<tag>/00_stacking_log.txt`, `01_stacking_summary.md`, `evaluations/graphical/stacking/<tag>/02_metrics_test.png`, `03_winner_actual_vs_predicted_test.png` (plus `02b_metrics_unseen.png`, `03b_winner_actual_vs_predicted_unseen.png` untuk `full`), `models/<tag>/stacking_winner.pkl`.

- [ ] **Step 1: Tulis seluruh isi script**

Isi `12-stacking-ensemble.py` lengkap:
...
- [ ] **Step 2: Verifikasi sintaks (tanpa menjalankan pipeline)**

Run:
```powershell
D:\Coding\#bigdata\venv\Scripts\python.exe -m py_compile 12-stacking-ensemble.py
```
Expected: tidak ada output (artinya kompilasi sukses), exit code 0.

---

### Task 2: Smoke run `--run wu`

**Files:**
- Tidak ada file baru dibuat di task ini (hanya menjalankan Task 1 dan verifikasi).

**Interfaces:**
- Consumes: output Task 1.
- Produces: `evaluations/statistical/stacking/wu/*`, `evaluations/graphical/stacking/wu/*`, `models/wu/stacking_winner.pkl`.

- [ ] **Step 1: User jalankan run `wu`**

Run (user, di cmd dengan venv aktif):
```bat
D:\Coding\#bigdata\venv\Scripts\activate.bat
python 12-stacking-ensemble.py --run wu
```
Expected, selesai dalam hitungan detik sampai beberapa menit (tidak ada training, hanya inference dan fit RidgeCV ringan), console mencetak seleksi varian per arsitektur, kombinasi pemenang, MAE test, hasil DM test, tanpa error.

- [ ] **Step 2: Verifikasi output**

Run:
```powershell
Get-ChildItem evaluations\statistical\stacking\wu, evaluations\graphical\stacking\wu, models\wu\stacking_winner.pkl
```
Expected: `00_stacking_log.txt`, `01_stacking_summary.md` di folder statistical, `02_metrics_test.png`, `03_winner_actual_vs_predicted_test.png` di folder graphical (run `wu` tidak punya unseen jadi tidak ada file `02b`/`03b`), `stacking_winner.pkl` ada.

- [ ] **Step 3: Baca `01_stacking_summary.md`, sanity check manual**

Cek, MAE base learner individu di tabel cocok kira kira dengan angka Stage 05/10/11 sebelumnya (boleh beda tipis karena beda subset evaluasi, tapi tidak boleh jomplang jauh), kombinasi pemenang beranggota 3 sampai 5 nama arsitektur valid, bobot Ridge (kalau menang) masuk akal (tidak NaN/Inf).

---

### Task 3: Full run `--run full`

**Files:**
- Tidak ada file baru dibuat di task ini.

**Interfaces:**
- Consumes: output Task 1.
- Produces: `evaluations/statistical/stacking/full/*`, `evaluations/graphical/stacking/full/*` (termasuk `02b_metrics_unseen.png`, `03b_winner_actual_vs_predicted_unseen.png`), `models/full/stacking_winner.pkl`.

- [ ] **Step 1: User jalankan run `full`**

Run (user, di cmd dengan venv aktif):
```bat
python 12-stacking-ensemble.py --run full
```
Expected, sama seperti Task 2 tapi dengan tambahan metrik unseen set di console dan laporan.

- [ ] **Step 2: Verifikasi output**

Run:
```powershell
Get-ChildItem evaluations\statistical\stacking\full, evaluations\graphical\stacking\full, models\full\stacking_winner.pkl
```
Expected: sama seperti Task 2 Step 2, plus `02b_metrics_unseen.png`, `03b_winner_actual_vs_predicted_unseen.png`.

- [ ] **Step 3: Sanity check manual**

Sama seperti Task 2 Step 3, plus cek bagian unseen di `01_stacking_summary.md`.

---

### Task 4: Commit script dan hasil eksperimen

**Files:**
- Modify: (tidak ada, commit murni menambahkan file baru)

- [ ] **Step 1: Commit**

Run:
```powershell
git add 12-stacking-ensemble.py evaluations/statistical/stacking evaluations/graphical/stacking models/full/stacking_winner.pkl models/wu/stacking_winner.pkl
git commit -m "feat: stacking ensemble, exhaustive combination search (3-5 members) over 8 base learners, both runs"
```
Expected: commit berhasil, tanpa baris co-author.

---

### Task 5: Update `todo-list.md` dan `final-report.md`
...
**Interfaces:**
- Consumes: `evaluations/statistical/stacking/{full,wu}/01_stacking_summary.md` (angka aktual hasil Task 2/3, dibaca ulang oleh agent sebelum menulis).

- [ ] **Step 1: Baca kedua `01_stacking_summary.md`**

Agent membaca `evaluations/statistical/stacking/full/01_stacking_summary.md` dan `evaluations/statistical/stacking/wu/01_stacking_summary.md` untuk mengambil angka aktual, kombinasi pemenang, dan hasil DM test kedua run. TIDAK menebak atau menghardcode angka sebelum file ini dibaca.

- [ ] **Step 2: Tambah Task 5 di `todo-list.md`**

Tambahkan section baru di akhir `todo-list.md` (sebelum bagian "Keputusan yang sudah terkunci"), format sama seperti Task 3b/3c yang sudah ada, checklist ditandai `[x]` untuk tiap sub-langkah yang sudah selesai, ringkasan hasil (kombinasi pemenang per run, menang/kalah vs model individu terbaik) ditulis di bawah judul task.

- [ ] **Step 3: Tambah Bagian 8 di `final-report.md`**

Tambahkan bagian baru "8. Stacking Ensemble" di akhir `final-report.md`, format konsisten dengan bagian bagian sebelumnya (subsection metodologi singkat, tabel hasil, interpretasi), isi angka dari hasil Step 1, sertakan catatan kejujuran soal risiko overfitting seleksi validation set (lihat spec, section "Catatan Kejujuran"), lint tanda baca sesuai Global Constraint 2 (tanpa dash naratif, ":" jadi list).

- [ ] **Step 4: Commit**

Run:
```powershell
git add todo-list.md final-report.md
git commit -m "docs: Task 5 stacking ensemble results in todo-list.md and final-report.md"
```
Expected: commit berhasil, tanpa baris co-author.
